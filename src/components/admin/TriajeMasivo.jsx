...
const COLOR_MAP = {
  colapsado:      ['#fee2e2','#991b1b'],
  riesgo_colapso: ['#fef3c7','#92400e'],
  danos_visibles: ['#ffedd5','#c2410c'],
  sin_danos:      ['#dcfce7','#166534'],
  sin_informacion:['#f5f3ff','#5b21b6'],
};
...
// ── Lightbox a pantalla completa (renderizado en raíz del componente principal) ─
function Lightbox({ fotos, idx, onClose, onPrev, onNext }) {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onClose}
        style={{ position: 'absolute', top: 16, right: 16, background: 'rgba(255,255,255,0.12)', border: 'none', borderRadius: '50%', width: 44, height: 44, color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}
      >
        <X size={20} />
      </button>
      ...
      {fotos.length > 1 && (
        <>
          <button
            ...
            style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', background: 'rgba(255,255,255,0.12)', border: 'none', borderRadius: '50%', width: 48, height: 48, color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}
          >
            <ChevronLeft size={24} />
          </button>
          <button
            onClick={e => { e.stopPropagation(); onNext(); }}
            style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'rgba(255,255,255,0.12)', border: 'none', borderRadius: '50%', width: 48, height: 48, color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10 }}
          >
      ...
      {/* Contador */}
      {fotos.length > 1 && (
        <div style={{ position: 'absolute', bottom: 20, left: '50%', transform: 'translateX(-50%)', background: 'rgba(0,0,0,0.65)', color: '#fff', borderRadius: 20, padding: '5px 16px', fontSize: 12, fontWeight: 700 }}>
          {idx + 1} / {fotos.length}
        </div>
      )}
    </div>
...
// ── Carrusel de fotos (miniatura — sin overflow hidden en la imagen) ───────────
function CarruselFotos({ fotos, nivelCfg, onAbrirLightbox }) {
  const [fotoIdx, setFotoIdx] = useState(0);

  if (!fotos || fotos.length === 0) {
    return (
      <div style={{ height: 180, background: nivelCfg.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
        <Building2 size={36} style={{ color: nivelCfg.color, opacity: 0.4 }} />
  ...
  return (
    <div style={{ height: 200, position: 'relative', background: '#111', overflow: 'hidden' }}>
      ...
      <button
        onPointerDown={e => { e.stopPropagation(); onAbrirLightbox(fotoIdx); }}
        style={{ position: 'absolute', bottom: 10, right: 10, background: 'rgba(0,0,0,0.65)', border: 'none', borderRadius: 8, padding: '6px 10px', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, zIndex: 6 }}
      >
        <ZoomIn size={13} /> Pantalla completa
      </button>
...
// ── Overlay de confirmación: COLAPSADO ────────────────────────────────────────
...
// ── Tarjeta de edificio (swipeable) ───────────────────────────────────────────
function TarjetaEdificio({ edificio, onAccion, procesando, onAbrirLightbox }) {
  ...
  const borderColor = swipeDir === 'left' ? '#991b1b' : swipeDir === 'right' ? '#166534' : cfg.border;

  return (
    <div style={{ position: 'relative', userSelect: 'none' }}>
      {swipeDir === 'right' && (
        <div style={{ position: 'absolute', top: 24, left: 20, zIndex: 10, background: '#166534', color: '#fff', borderRadius: 10, padding: '6px 14px', fontWeight: 800, fontSize: 13, transform: 'rotate(-12deg)', boxShadow: '0 4px 14px rgba(0,0,0,0.2)', pointerEvents: 'none' }}>✅ SIN DAÑOS</div>
      )}
      {swipeDir === 'left' && (
        <div style={{ position: 'absolute', top: 24, right: 20, zIndex: 10, background: '#991b1b', color: '#fff', borderRadius: 10, padding: '6px 14px', fontWeight: 800, fontSize: 13, transform: 'rotate(12deg)', boxShadow: '0 4px 14px rgba(0,0,0,0.2)', pointerEvents: 'none' }}>💥 COLAPSADO</div>
      )}

      <div
        onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerLeave={onPointerUp}
        style={{ background: '#fff', borderRadius: 20, border: `2.5px solid ${borderColor}`, overflow: 'hidden', boxShadow: '0 6px 28px rgba(0,0,0,0.10)', transform: `translateX(${dragX}px) rotate(${dragX * 0.04}deg)`, transition: dragging ? 'none' : 'transform 300ms ease, border-color 150ms', cursor: dragging ? 'grabbing' : 'grab', touchAction: 'none' }}
      >
        {/* Foto + badges */}
        <div style={{ position: 'relative' }}>
          <CarruselFotos fotos={fotos} nivelCfg={cfg} onAbrirLightbox={onAbrirLightbox} />
          <div style={{ position: 'absolute', top: 10, left: 10, background: cfg.bg, color: cfg.color, border: `1px solid ${cfg.border}`, borderRadius: 20, padding: '4px 10px', fontSize: 10, fontWeight: 800, zIndex: 6, pointerEvents: 'none' }}>
            {cfg.label}
          </div>
          {(edificio.riesgo_colapso || edificio.triage_riesgo === 'riesgo_colapso') && (
            <div style={{ position: 'absolute', top: 36, left: 10, background: '#450a0a', color: '#fca5a5', border: '1px solid #7f1d1d', borderRadius: 20, padding: '3px 8px', fontSize: 9, fontWeight: 800, zIndex: 6, display: 'flex', alignItems: 'center', gap: 3, pointerEvents: 'none' }}>
              <AlertTriangle size={9} /> RIESGO COLAPSO
            </div>
          )}
        </div>
        ...
        <div style={{ padding: '14px 18px 16px' }}>
          ...
          {edificio.triage_estado && edificio.triage_estado !== 'pendiente_triage' && (
            <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 8, padding: '5px 10px', marginBottom: 6, display: 'inline-flex', alignItems: 'center', gap: 5 }}>
              ...
              {edificio.triage_por && <span style={{ fontSize: 9, color: '#15803d' }}>· por {edificio.triage_por}</span>}
            </div>
          )}
...
// ── Panel contadores ───────────────────────────────────────────────────────────
function PanelContadores({ titulo, items, datos, columnas = 3 }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${columnas}, 1fr)`, gap: 5 }}>
      {items.map(s => (
        <div key={s.key} style={{ background: s.bg, borderRadius: 9, padding: '7px 4px', textAlign: 'center' }}>
          <p style={{ fontSize: 18, fontWeight: 800, color: s.color, margin: 0 }}>{datos[s.key] ?? 0}</p>
...
export default function TriajeMasivo() {
  ...
  // Lightbox: { fotos: [], idx: number } | null
  ...
  const solicitarAccion = (accion) => {
    ...
    if (accion === 'colapsado') { setAccionPendiente('colapsado'); setOverlayColapsado(true); }
    else ejecutarAccion(accion, '', true);
  };
  ...
  const ejecutarAccion = async (accion, notas = '', notificar = true) => {
    ...
    if (accion === 'colapsado') {
      Object.assign(updateData, { triage_riesgo: 'riesgo_colapso', nivel_dano: 'colapsado', estado_acceso: 'clausurado', riesgo_colapso: true, prioridad: 'critica', requiere_inspeccion_presencial: false });
      if (notas) updateData.triage_notas = notas;
    } else if (accion === 'riesgo_colapso') {
      Object.assign(updateData, { triage_riesgo: 'riesgo_colapso', riesgo_colapso: true, estado_acceso: 'solo_rescatistas', prioridad: 'critica' });
      if (!['critico', 'colapsado'].includes(edificio.nivel_dano)) updateData.nivel_dano = 'critico';
    } else if (accion === 'danos_visibles') {
      Object.assign(updateData, { triage_riesgo: 'riesgo_moderado', estado_acceso: 'entrada_limitada', prioridad: 'alta' });
      if (!['grave', 'critico', 'colapsado'].includes(edificio.nivel_dano)) updateData.nivel_dano = 'grave';
    } else if (accion === 'sin_danos') {
      Object.assign(updateData, { triage_riesgo: 'solo_estetico', estado_acceso: 'entrada_autorizada', prioridad: 'normal' });
      if (edificio.nivel_dano === 'no_evaluado' || !edificio.nivel_dano) updateData.nivel_dano = 'leve';
    } else if (accion === 'sin_informacion') {
      Object.assign(updateData, { triage_riesgo: 'sin_clasificar', prioridad: 'normal' });
    }
    ...
    try {
      ...
      if (notificar && accion !== 'sin_informacion') {
        ...
        base44.functions.invoke('notificarActualizacionEdificio', {
          edificio_id: edificio.id, tipo_accion: 'estado_cambiado',
          nivel_dano: updateData.nivel_dano || edificio.nivel_dano,
          direccion: edificio.direccion, nombre_lugar: edificio.nombre_lugar,
          descripcion: descMap[accion] || 'Estado actualizado en triaje masivo.', lang: 'es',
        }).catch(() => {});
      }
      ...
      setContadoresGlobales(prev => {
        ...
        next[accion === 'sin_informacion' ? 'sin_informacion' : accion] = (next[accion === 'sin_informacion' ? 'sin_informacion' : accion] || 0) + 1;
        return next;
      });
      setIdx(prev => prev + 1);
      mostrarToast(ACCION_LABELS[accion] || 'Guardado');
    } catch (e) {
  ...
  return (
    <div style={{ maxWidth: 560, margin: '0 auto', fontFamily: 'system-ui, sans-serif', position: 'relative' }}>
      ...
      {/* ── Lightbox pantalla completa (montado en raíz, fuera de cualquier overflow) */}
      {lightbox && (
        <Lightbox
          fotos={lightbox.fotos}
          idx={lightbox.idx}
          onClose={() => setLightbox(null)}
          onPrev={() => setLightbox(lb => ({ ...lb, idx: (lb.idx - 1 + lb.fotos.length) % lb.fotos.length }))}
          onNext={() => setLightbox(lb => ({ ...lb, idx: (lb.idx + 1) % lb.fotos.length }))}
        />
      )}

      {/* ── Toast */}
      {toastMsg && (
        <div style={{ position: 'fixed', bottom: 80, left: '50%', transform: 'translateX(-50%)', zIndex: 300, padding: '10px 20px', borderRadius: 14, fontWeight: 700, fontSize: 13, background: toastMsg.tipo === 'error' ? '#fee2e2' : '#dcfce7', color: toastMsg.tipo === 'error' ? '#991b1b' : '#166534', boxShadow: '0 4px 20px rgba(0,0,0,0.15)', display: 'flex', alignItems: 'center', gap: 8, whiteSpace: 'nowrap' }}>
      ...
      {!cargando && (
        <div style={{ marginBottom: 16, background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 14, padding: '12px 14px' }}>
          ...
          <PanelContadores items={ITEMS_BD} datos={contadoresGlobales} columnas={3} />
        </div>
      )}
      ...
      {procesados.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          ...
          <PanelContadores items={ITEMS_SESION} datos={stats} columnas={5} />
        </div>
      )}
      ...
      {cargando ? (
        <div style={{ textAlign: 'center', padding: '60px 0' }}>
          ...
          <p style={{ fontSize: 13, color: '#9ca3af', marginTop: 12 }}>Cargando edificios...</p>
        </div>
      ) : total === 0 ? (
        <div style={{ textAlign: 'center', padding: '50px 20px', background: '#f0fdf4', borderRadius: 16, border: '1px solid #bbf7d0' }}>
          <p style={{ fontSize: 36, margin: '0 0 10px' }}>✅</p>
          <p style={{ fontSize: 14, fontWeight: 700, color: '#15803d', margin: '0 0 12px' }}>¡Cola vacía!</p>
        ...
        <>
          ...
          {/* Tarjeta — pasa onAbrirLightbox para elevar el estado arriba del overflow */}
          <TarjetaEdificio
            edificio={edificio}
            onAccion={solicitarAccion}
            procesando={procesando}
            onAbrirLightbox={(fotoIdx) => setLightbox({ fotos: edificio.foto_urls || [], idx: fotoIdx })}
          />
        ...
        <div style={{ marginTop: 20 }}>
          ...
          {mostrarHistorial && (
            <div style={{ marginTop: 6, border: '1px solid #e5e7eb', borderRadius: 10, overflow: 'hidden' }}>
              {procesados.map((p, i) => {
                const [bg, clr] = COLOR_MAP[p.accion] || ['#f3f4f6','#6b7280'];
