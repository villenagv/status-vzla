...
export default function ActualizacionPersonaRapida({ persona, es, onClose, onSaved }) {
  const [tipo, setTipo] = useState(persona._modo === 'encontrado' ? 'encontrado' : 'busqueda');
  const [ubicacion, setUbicacion] = useState(persona.ultima_ubicacion_conocida || '');
  const [contacto, setContacto] = useState('');
  const [reportante, setReportante] = useState('');
  const [condicion, setCondicion] = useState(persona.condicion && persona.condicion !== 'no_sabe' ? persona.condicion : 'a_salvo');
  const [notas, setNotas] = useState('');
  const [enviando, setEnviando] = useState(false);
  const [error, setError] = useState('');

  const estadoDesdeCondicion = () => {
    if (tipo === 'busqueda') return 'buscando';
    if (condicion === 'a_salvo') return 'encontrado_con_vida';
    if (condicion === 'no_identificado') return 'informacion_recibida';
    if (condicion === 'fallecido_reportado') return 'fallecido_reportado';
    return 'en_hospital_refugio';
  };
  ...
  const enviar = async () => {
    ...
    const estadoNuevo = estadoDesdeCondicion();
    const mensaje = notas.trim() || (tipo === 'encontrado'
      ? (es ? 'Reportaron que la persona fue encontrada.' : 'The person was reported found.')
      : (es ? 'La persona sigue en búsqueda o necesita actualización de datos.' : 'The person is still being searched for or needs updated details.'));
    ...
    try {
      if (tipo === 'encontrado') {
        await base44.entities.PersonasEncontradas.create({
          ...
          nivel_verificacion: persona._fuente === 'institucional' ? 'institucional' : 'comunidad',
          fuente: persona._fuente === 'institucional' ? 'actualizacion_institucional' : 'web_publica',
        });
      }

      if (esBusqueda) {
        await base44.entities.PersonasBuscadas.update(persona.id, { estado_caso: estadoNuevo, ultima_ubicacion_conocida: ubicacion.trim(), notas_publicas: mensaje });
        ...
        await base44.entities.PersonasBuscadas.create({
          ...
          sexo: persona.sexo || '',
          descripcion_fisica: persona.descripcion_fisica || mensaje,
          ultima_ubicacion_conocida: ubicacion.trim(),
          ciudad: persona.ciudad || 'No indicada',
          estado_region: persona.estado_region || 'No indicado',
          contacto_nombre: reportante.trim(),
          contacto_telefono: contacto.trim(),
          estado_caso: 'buscando',
          notas_publicas: mensaje,
          fuente: 'actualizacion_desde_directorio',
        });
      }

      if (persona._fuente === 'institucional' && persona._rawId) {
        await base44.entities.PersonaRegistrada.update(persona._rawId, {
          condicion: tipo === 'encontrado' ? condicion : 'no_sabe',
          observaciones: mensaje,
        });
      }

      if (persona._fuente === 'cris' && persona._rawId) {
        await base44.entities.PersonaCRIS.update(persona._rawId, {
          estado_actual: tipo === 'encontrado' ? (condicion === 'a_salvo' ? 'encontrado' : 'en_hospital') : 'buscado_por_familiar',
          ubicacion_texto: ubicacion.trim(),
          notas_publicas: mensaje,
        });
      }

      await base44.functions.invoke('notificarTodo', { tipo: 'persona', entidad_id: persona.id, nombre: persona.nombre_completo, estado_label: tipo === 'encontrado' ? (es ? 'Encontrada con nueva ubicación' : 'Found with new location') : (es ? 'Sigue en búsqueda' : 'Still being searched for'), estado_color: tipo === 'encontrado' ? '#2E7D32' : '#B83A52', ubicacion: ubicacion.trim(), mensaje, contactos: [{ nombre: reportante.trim() || (es ? 'Reportante' : 'Reporter'), telefono: contacto.trim() }], lang: es ? 'es' : 'en' }).catch(() => {});
      onSaved?.({ ...persona, estado_caso: estadoNuevo, ultima_ubicacion_conocida: ubicacion.trim(), descripcion_fisica: mensaje });
      onClose();
  ...
  return <div className="fixed inset-0 z-[120] bg-black/50 flex items-end sm:items-center justify-center p-3">
    <div className="bg-white rounded-2xl w-full max-w-md p-4 shadow-xl">
      ...
      <div className="grid grid-cols-2 gap-2 mb-3"><button onClick={() => setTipo('busqueda')} className={`rounded-xl py-2 text-xs font-bold border ${tipo === 'busqueda' ? 'bg-[#B83A52] text-white border-[#B83A52]' : 'bg-white text-gray-600 border-gray-200'}`}>{es ? 'Sigue en búsqueda' : 'Still searching'}</button><button onClick={() => setTipo('encontrado')} className={`rounded-xl py-2 text-xs font-bold border ${tipo === 'encontrado' ? 'bg-green-600 text-white border-green-600' : 'bg-white text-gray-600 border-gray-200'}`}>{es ? 'Está encontrada' : 'Found'}</button></div>
      {tipo === 'encontrado' && <select value={condicion} onChange={e => setCondicion(e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm mb-2"><option value="a_salvo">{es ? 'A salvo' : 'Safe'}</option><option value="herido_leve">{es ? 'Herida leve' : 'Minor injury'}</option><option value="herido_grave">{es ? 'Herida grave' : 'Serious injury'}</option><option value="fallecido_reportado">{es ? 'Fallecimiento reportado' : 'Death reported'}</option><option value="no_identificado">{es ? 'No estoy seguro' : 'Not sure'}</option></select>}
      ...
      <button onClick={enviar} disabled={enviando} className="w-full bg-[#1A1F2E] text-white rounded-xl py-3 text-sm font-black disabled:opacity-50">{enviando ? (es ? 'Enviando...' : 'Sending...') : (es ? 'Guardar actualización' : 'Save update')}</button>
    </div>
  </div>;
}